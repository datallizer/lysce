    ...
    <select class="form-select bg-warning" name="tipoFtl">
        ...
        if (mysqli_num_rows($result) > 0) {
            while ($registro = mysqli_fetch_assoc($result)) {
                echo "<option value='{$registro['nombreServicio']}'>{$registro['nombreServicio']}</option>";
            }
        }
        ?>
    </select>
...
<!-- ORIGEN -->
<div class="row">
    <div class="col-md-3 p-3" style="border: 1px solid #666666;">
        ...
        <select class="form-select" name="idOrigen" id="origen">
            ...
            if (mysqli_num_rows($result) > 0) {
                while ($registro = mysqli_fetch_assoc($result)) {
                    echo "<option value='{$registro['id']}'>{$registro['proveedor']}</option>";
                }
            }
            ?>
        </select>
    ...
    <!-- PRIMER DESCARGA (NUEVO CAMPO) -->
    <div class="col-md-3 p-3" style="border: 1px solid #666666;">
        <p class="mb-1"><b>Primer Descarga</b></p>
        <select class="form-select" name="idPrimerDescarga" id="primerDescarga">
            <option selected>Selecciona la primer descarga</option>
            <?php
            $query = "SELECT * FROM proveedores WHERE estatus = 1";
            $result = mysqli_query($con, $query);
            if (mysqli_num_rows($result) > 0) {
