...
if (isset($_POST['delete'])) {
    ...
    $delete_gastos = "DELETE FROM gastosfcl WHERE idFcl='$id'";
    $delete_incrementables = "DELETE FROM incrementablesfcl WHERE idFcl='$id'";
    $delete_descripcion = "DELETE FROM descripcionmercanciasfcl WHERE idFcl='$id'";
    $delete_destino = "DELETE FROM gastosdestinofcl WHERE idFcl='$id'";
    $delete_origen = "DELETE FROM gastosorigenfcl WHERE idFcl='$id'";

    $delete_fcl = "DELETE FROM fcl WHERE id='$id'";
    ...
    $query_run_fcl = mysqli_query($con, $delete_fcl);

    if ($query_run_gastos && $query_run_incrementables && $query_run_descripcion && $query_run_destino && $query_run_origen && $query_run_fcl) {
        $_SESSION['alert'] = [
            'title' => 'ELIMINADO EXITOSAMENTE',
            'icon' => 'success'
        ];
        header("Location: fcl.php");
        exit(0);
    } else {
        $_SESSION['alert'] = [
...
if (isset($_POST['save'])) {
    ...
    $tipoFcl = get_post_value($con, 'tipoFcl', '');
    ...
    $sql = "INSERT INTO fcl (
        ...
        totalCotizacionTexto, tipoFcl, observaciones, lugarOrigen, lugarDestino, porcentajeSeguro
    ) VALUES (
    ...
    if ($query_run) {
        ...
        for ($i = 0; $i < count($cantidad); $i++) {
            ...
            $sql_detalle = "INSERT INTO descripcionmercanciasfcl (
                idFcl, cantidad, unidadMedida, descripcion, 
                largoCm, anchoCm, altoCm, largoPlg, anchoPlg, altoPlg, 
                piesCubicos, metrosCubicos, libras, kilogramos, valorFactura
            ) VALUES (
                '$idFcl', '$cantidad_val', '$unidadMedida_val', '$descripcion_val', 
        ...
        if (!empty($_POST['gastosOrigen'])) {
            ...
            for ($i = 0; $i < count($gastosOrigen); $i++) {
                ...
                $sql_origen = "INSERT INTO gastosorigenfcl (idFcl, gastosOrigen, euros, equivalenciaOrigen, usdOrigen)
                               VALUES ('$idFcl', '$origenes', '$euro', '$equiv', '$usds')";
        ...
        if (!empty($_POST['gastoDestino'])) {
            ...
            for ($i = 0; $i < count($gastoDestino); $i++) {
                ...
                $sql_destino = "INSERT INTO gastosdestinofcl (idFcl, gastoDestino, usdDestino, mxnDestino) VALUES ('$idFcl', '$destinos', '$usDestino', '$mxDestino')";
        ...
        if (!empty($_POST['incrementable'])) {
            ...
            for ($i = 0; $i < count($incrementables); $i++) {
                ...
                $sql_incrementable = "INSERT INTO incrementablesfcl (idFcl, incrementable, incrementableUsd, incrementableMx) 
                                      VALUES ('$idFcl', '$incrementable', '$incrementableUsd', '$incrementableMx')";
        ...
        if (!empty($_POST['conceptoGasto'])) {
            ...
            for ($i = 0; $i < count($conceptoGasto); $i++) {
                ...
                $sql_gasto = "INSERT INTO gastosfcl (idFcl, conceptoGasto, montoGasto, montoGastoMx, ivaGasto) VALUES ('$idFcl', '$concepto', '$monto', '$montoMx', '$iva')";
...
if (isset($_POST['update'])) {
    ...
    $tipoFcl = mysqli_real_escape_string($con, $_POST['tipoFcl']);
    ...
    if ($query_run) {
        ...
        mysqli_query($con, "DELETE FROM descripcionmercanciasfcl WHERE idFcl = '$id'");
        mysqli_query($con, "DELETE FROM incrementablesfcl WHERE idFcl = '$id'");
        mysqli_query($con, "DELETE FROM gastosorigenfcl WHERE idFcl = '$id'");
        mysqli_query($con, "DELETE FROM gastosdestinofcl WHERE idFcl = '$id'");
        mysqli_query($con, "DELETE FROM gastosfcl WHERE idFcl = '$id'");
        ...
        if (!empty($_POST['gastoDestino']) && is_array($_POST['gastoDestino'])) {
            foreach ($_POST['gastoDestino'] as $i => $concepto) {
                ...
                $sql_destino = "INSERT INTO gastosdestinofcl (idFcl, gastoDestino, usdDestino, mxnDestino)
        ...
        if (!empty($_POST['conceptoGasto']) && is_array($_POST['conceptoGasto'])) {
            foreach ($_POST['conceptoGasto'] as $i => $gasto) {
                ...
                $sql_gasto = "INSERT INTO gastosfcl (idFcl, conceptoGasto, montoGasto, montoGastoMx, ivaGasto)
